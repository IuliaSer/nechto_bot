...
//      String targetUrl = publicHttps + "/telegram/webhook";

      String current = getCurrentWebhookUrl();
      if (!targetUrl.equals(current)) {
        setWebhook(targetUrl);
        System.out.println("[webhook] setWebhook -> " + targetUrl);
      } else {
        System.out.println("[webhook] already set: " + current);
      }
    } catch (Exception e) {
      // Логируем, но не валим приложение
      System.err.println("[webhook] failed to register: " + e.getMessage());
    }
  }
  ...
  public void clearUpdates() {
    try {
      ...
      if (!resp.getStatusCode().is2xxSuccessful()) {
        throw new IllegalStateException("Clear failed: " + resp.getStatusCode() + " "
                + resp.getBody());
      }
